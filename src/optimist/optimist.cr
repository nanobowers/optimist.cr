module Optimist
  ...
  # If we ever create a parser, and then call `Optimist.die` external to the
  ...
  # (`Parser.opt`), zero or more calls to `banner` (`Parser.banner`), and
  # probably a call to `version` (`Parser.version`).
  #
  # The returned block contains a value for every option specified with
  # `opt`.  The value will be the value given on the commandline, or the
  # default value if the option was not specified on the commandline. For
  # every option specified on the commandline, a boolean property named
  # `given?` will be set to true/false.
  #
  # ## Example
  # ``` crystal
  # require "optimist"
  ...
  #
  # ## Settings
  # `Optimist.options` and `Optimist::Parser.new` accept keyword args
  # to control how options are interpreted, e.g:.
  #
  # ```crystal
  ...
  #   o = par.parse ARGV
  #   raise Optimist::HelpNeeded.new() if ARGV.empty? # show help screen
  #   o
  # end
  # ```
